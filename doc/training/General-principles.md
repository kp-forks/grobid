...
For generating pre-annotated training files for Grobid based on the existing models, there are two ways. The recommended way is to use the API [here](../Grobid-service.md#generate-training-data). The alternative is to use the batch mode [here](../Training-the-models-of-Grobid.md#generation-of-training-data) and [here](../Grobid-batch.md#createtraining).

After running the procedure `createTraining` (either via API or batch-mode) on a set of PDF files using methods for creating training data, each article comes with:
